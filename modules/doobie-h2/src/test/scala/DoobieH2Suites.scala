...
final class CoalesceSuite extends DoobieH2DatabaseSuite with SqlCoalesceSuite {
  ...
  def mapping: IO[(Mapping[IO], SqlStatsMonitor[IO,Fragment])] =
    DoobieMonitor.statsMonitor[IO].map(mon => (new DoobieH2TestMapping(transactor, mon) with SqlCoalesceMapping[IO], mon))
}
...
final class ComposedWorldSuite extends DoobieH2DatabaseSuite with SqlComposedWorldSuite {
  def mapping: IO[(CurrencyMapping[IO], Mapping[IO])] =
    ...
    } yield (currencyMapping, new SqlComposedMapping(new DoobieH2TestMapping(transactor) with SqlWorldMapping[IO], currencyMapping))
}
...
final class FilterOrderOffsetLimitSuite extends DoobieH2DatabaseSuite with SqlFilterOrderOffsetLimitSuite {
  lazy val mapping = new DoobieH2TestMapping(transactor) with SqlFilterOrderOffsetLimitMapping[IO]
}

final class FilterOrderOffsetLimit2Suite extends DoobieH2DatabaseSuite with SqlFilterOrderOffsetLimit2Suite {
  lazy val mapping = new DoobieH2TestMapping(transactor) with SqlFilterOrderOffsetLimit2Mapping[IO]
}
...
final class MappingValidatorValidSuite extends DoobieH2DatabaseSuite with SqlMappingValidatorValidSuite {
...
final class MappingValidatorInvalidSuite extends DoobieH2DatabaseSuite with SqlMappingValidatorInvalidSuite {
  // no DB instance needed for this suite
...
final class RecursiveInterfacesSuite extends DoobieH2DatabaseSuite with SqlRecursiveInterfacesSuite {
  lazy val mapping =
...
final class WorldCompilerSuite extends DoobieH2DatabaseSuite with SqlWorldCompilerSuite {
  ...
  def mapping: IO[(Mapping[IO], SqlStatsMonitor[IO,Fragment])] =
    DoobieMonitor.statsMonitor[IO].map(mon => (new DoobieH2TestMapping(transactor, mon) with SqlWorldMapping[IO], mon))
